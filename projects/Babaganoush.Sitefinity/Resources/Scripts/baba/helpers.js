...
define([
    ...
    return {
        ...
        getCurrentPage: function () {
            ...
            //REMOVE VIRTUAL APP PATH IF APPLICABLE
            if (page == _.trim(baseUrl.toLowerCase(), '/'))
                page = '';
            return page;
        },
        ...
        urlExists: function (url) {
            //TODO: ADDS 404 ERROR TO CONSOLE
        ...
        minifyHtml: function (value) {
            //REMOVES SPACES BETWEEN ELEMENTS AND NEW LINES
        ...
        treeify: function (list, options) {
            ...
            //CREATE LIST BASED ON IDS AS KEYS
            list.forEach(function(item) {
                lookup[item[settings.idAttr]] = item;
                item[settings.childrenAttr] = [];
            });

            //SEARCH BY KEYS TO STORE CHILDREN
            list.forEach(function(item) {
                if (item[settings.parentAttr]) {
                    lookup[item[settings.parentAttr]]
                        [settings.childrenAttr].push(item);
                } else {
                    treeList.push(item);
                }
            });
