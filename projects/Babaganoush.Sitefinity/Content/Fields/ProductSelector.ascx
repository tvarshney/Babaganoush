...
<div id="selectorWrapper" runat="server">
    <sf:FlatSelector ID="itemsSelector" runat="server" ItemType="Telerik.Sitefinity.Ecommerce.Catalog.Model.Product"
        ...
        <DataMembers>
            <sf:DataMemberInfo runat="server" Name="ThumbnailUrl" IsExtendedSearchField="false" HeaderText=''>
                <img sys:src="{{Item.Thumbnail.ThumbnailUrl}}" width="48px" />
            </sf:DataMemberInfo>
            <sf:DataMemberInfo runat="server" Name="Title" IsExtendedSearchField="true" HeaderText='Title'>
                <strong>{{ Item.Title.Value }}</strong>
            </sf:DataMemberInfo>
            <sf:DataMemberInfo runat="server" Name="ProductTypeTitle" IsExtendedSearchField="true" HeaderText='Type'>
                <strong>{{ ProductTypeTitle }}</strong>
            </sf:DataMemberInfo>
        </DataMembers>
    </sf:FlatSelector>
...
<ul id="selectedItemsList" data-template="ul-template-EventsSelectorFieldControl" data-bind="source: items" runat="server" class="sfCategoriesList"></ul>
<script id="ul-template-EventsSelectorFieldControl" type="text/x-kendo-template">
    <li>
        <span data-bind="text: Item.Title.Value, attr: {data-id: Id}"> </span>
